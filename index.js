...
app.get("/thingamabobs/:id", (req, res) => {
  if (typeof thingamabobs[req.params.id - 1] === "undefined") {
    return res
      .status(404)
      .send({ error: "Object not found. Check your thingamabobs id" });
  }
...
app.post("/thingamabobs", (req, res) => {
  if (!req.body.name || !req.body.price) {
    return res
      .status(400)
      .send({ error: "One or multiple parameters missing" });
  }
  ...
  res
    .status(201)
    .location("localhost:8080/thingamabobs/" + (thingamabobs.length - 1))
    .send(newThingy);
});
...
app.get("/people/:id", (req, res) => {
  if (typeof people[req.params.id - 1] === "undefined") {
    return res
      .status(404)
      .send({ error: "Object not found. Check your people id" });
  }
...
app.post("/people", (req, res) => {
  ...
  res
    .status(201)
    .location("localhost:8080/people/" + (people.length - 1))
    .send(peopleThingy);
});
